...
**3. The gate.** Wire the verification tool into the gate so a draft with an unsupported claim is blocked, and the gate reports which claim failed and why. When the verification tool returns "uncertain," what the gate does is yours to decide — make the call deliberately. The rule checks (disclaimer, length, well-formed sections) are done for you.
